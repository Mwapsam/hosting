...
<?php
$box_classes = array('first','second','third','fourth','fifth');
$box_id = array('large','medium','small','micro','nano');
$cnt_plans = count($index_plans);
if($cnt_plans == 4 or $cnt_plans == 3)
{
	$br=0;
	foreach($index_plans as $single_plan){
		...
		?>
                <div id="shared-box-<?php echo $box_id[$cnt_plans-1];?>" class="<?php echo $box_classes[$br];?>">
	...
	<?php } ?>
					...
					<div id="box-content">
                        ...
                        <?php }elseif($single_plan['type'] == 'vps'){ ?>
						<ul>
							<li><strong><?php echo $plan['services']['processor']?></strong> CPU</li>
                        ...
                        <?php }elseif($single_plan['type'] == 'vps_kvm' or $single_plan['type'] == 'vps_openvz'){ ?>
						<ul>
...
}elseif($cnt_plans == 5 or $cnt_plans == 6){
	$br=0;
	foreach($index_plans as $single_plan){
		if($br == 3){
			$br = 0;
...
<?php } ?>
	...
	foreach($index_plans as $single_plan){
		...
		$plan['page_url'] = $single_plan['page_url'];
		?>
                <div id="shared-box-large" class="<?php echo $box_classes[$br];?>">
					<div id="box-title"><h4><a href="<?php echo $plan['page_url'];?>"><?php echo $plan['name'];?></a></h4></div>
					<div id="box-price">
							<?php if (!empty(rp_store_promos($plan['rp_product_id']))) { 
		$promo = rp_store_promos($plan['rp_product_id']);
