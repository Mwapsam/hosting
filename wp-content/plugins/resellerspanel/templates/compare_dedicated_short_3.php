<div class="article2-text-plan-area">
    <?php foreach($rp_plans as $key=>$plan) : ?>
        <div class="article1-plan-box<?php if(!empty($rp_best_plan) and $key == (int)$rp_best_plan){ echo ' article-plan-best'; }?>">
            ...
            <div class="article1-plan-features"><strong><?php echo $plan['services']['processor']?></strong><br><strong><?php echo ($plan['services']['ram']).' GB '.$plan['services']['ram_type']; ?></strong> RAM<br> <?php if($plan['services']['hdd'] == 1) echo '<strong>'.($plan['services']['disk_space']/1024000).' GB '.$plan['services']['hdd_type'].'</strong> Hard Drive'; else echo '<strong>'.$plan['services']['hdd'].'x'.($plan['services']['disk_space']/1024000).' GB '.$plan['services']['hdd_type'].'</strong> Hard Drives'; ?></div>
