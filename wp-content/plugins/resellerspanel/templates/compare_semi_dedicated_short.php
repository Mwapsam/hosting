  <table id="rp_presentation">
    ...
    <?php $i=0; foreach($rp_plans as $key=>$plan) : $i++; ?>
      <td class="rp_pr_plan<?php if($i==1){ echo ' first-plan'; } if(!empty($rp_best_plan) and $key == (int)$rp_best_plan){ echo ' rp_pr_best'; }?>">	<?php if (!empty(rp_store_promos($plan['rp_product_id']))) { 
