...
function theme_activation_function(){
	if(get_option('rp_home_hosting_deluxe')) update_option('page_on_front', get_option('rp_home_hosting_deluxe'));
}
...
function rpwp_theme_scripts() {
	...
	if(in_array($GLOBALS['wp_query']->post->ID,$arrOrders)){
		if(version_compare(get_bloginfo('version'), '3.5', '>=')){
			...
			wp_register_script('jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.9.2/jquery-ui.min.js');
	...
	wp_enqueue_script('modernizr_webp', get_bloginfo('template_url').'/js/modernizr-custom.js');
...
function hostingdeluxe_custom_logo_setup() {
	$defaults = array(
		'height'      => 80,
		'width'       => 300,
		'flex-height' => true,
...
add_action( 'after_setup_theme', 'hostingdeluxe_custom_logo_setup' );
